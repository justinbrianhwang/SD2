    ...
    def run_step(
        ...
        try:
            with self.torch.no_grad():
                encoding = [self.net.image_encoder(list(self.input_buffer["rgb"]))]
        ...
        control = self.modules.carla.VehicleControl()
        control.steer = e2e.scalar_float(steer)
        control.throttle = e2e.scalar_float(throttle)
        control.brake = e2e.scalar_float(brake)
        ...
        if not self.logged_shapes:
            LOGGER.info(
                "First AIM model output shapes: %s",
                {
                    ...
                    "pred_wp": e2e.shape_summary(pred_wp),
